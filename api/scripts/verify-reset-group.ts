...
async function main() {
  ...
  // --- 5. Resetting a Launched/Approved group must fully "unlaunch" it —
  // the bug report this test guards against: the text was reverting, but
  // lifecycleStatus stayed put, so the Chair/NA kept seeing the group in
  // whatever review/approval state it was already in, and the Admin's own
  // detail view kept showing the stale AI-generated text (latestDnaVersion)
  ...
  await prisma.group.update({
    where: { id: groupWithSnapshot.id },
    data: {
      lifecycleStatus: 'Approved',
      pendingReapproval: false,
      approvedFields: ['GroupProfile', 'MemberProfile', 'CompaniesProfile'],
      networkAdvisorEmail: naEmail,
      name: 'Edited Again',
      groupProfile: 'AI-GENERATED GROUP TEXT',
    },
  })
  const dnaVersion2 = await prisma.dnaVersion.create({
    data: {
      groupId: groupWithSnapshot.id,
      versionNumber: 2,
      content: { groupProfile: 'AI-GENERATED GROUP TEXT', memberProfile: 'ORIGINAL MEMBER TEXT', companiesProfile: 'ORIGINAL COMPANIES TEXT' },
      author: 'Ai',
      scoreStage: 'AutoGenerated',
    },
  })
  const naComment = await prisma.comment.create({
    data: { groupId: groupWithSnapshot.id, dnaVersionId: dnaVersion2.id, field: 'GroupProfile', author: 'NetworkAdvisor', text: 'old NA comment from before the reset' },
  })
  const chatTurn = await prisma.aiConversationTurn.create({
    data: { groupId: groupWithSnapshot.id, field: 'GroupProfile', chairEmail, role: 'Chair', messageText: 'old chair chat from before the reset', outcome: 'None' },
  })

  res = await call('/api/resetGroup', { method: 'POST', email: adminEmail, body: { groupId: groupWithSnapshot.id } })
  assert(res.status === 200, `expected 200 resetting an Approved/launched group, got ${res.status}: ${JSON.stringify(res.json)}`)
  assert(res.json.pendingReapproval === false, 'pendingReapproval is always false after Reset — the group is no longer Approved at all, not "approved but needs another look"')

  const groupAfterReset = await prisma.group.findUniqueOrThrow({ where: { id: groupWithSnapshot.id } })
  assert(groupAfterReset.lifecycleStatus === 'Imported', 'Reset fully unlaunches the group back to Imported')
  assert(groupAfterReset.approvedFields.length === 0, 'approvedFields cleared — none of the old approvals apply to the restored text')
  assert(groupAfterReset.groupProfile === 'ORIGINAL GROUP TEXT', "the group's live groupProfile is restored, not the stale AI-generated text")

  const latestVersionAfterReset = await prisma.dnaVersion.findFirst({ where: { groupId: groupWithSnapshot.id }, orderBy: { versionNumber: 'desc' } })
  assert(latestVersionAfterReset!.versionNumber === 3, 'a new (3rd) DnaVersion was created for the reset')
  assert(latestVersionAfterReset!.author === null && latestVersionAfterReset!.scoreStage === 'Imported', 'the reset version is unauthored, Imported-stage — matching a fresh import')
  const resetVersionContent = latestVersionAfterReset!.content as { groupProfile: string }
  assert(resetVersionContent.groupProfile === 'ORIGINAL GROUP TEXT', "the new version's own content matches the restored snapshot")

  assert((await prisma.comment.findUnique({ where: { id: naComment.id } })) === null, 'the old NA comment is gone after Reset')
  assert((await prisma.aiConversationTurn.findUnique({ where: { id: chatTurn.id } })) === null, 'the old chat history is gone after Reset')

  // The Admin's own detail view must show the restored text — before this
  // fix, getGroup's latestDnaVersion (what the "Current" tab actually
  // displays) stayed on the stale AI-generated version forever.
  res = await call(`/api/getGroup?groupId=${groupWithSnapshot.id}`, { method: 'GET', email: adminEmail })
  assert(res.json.groupProfile === 'ORIGINAL GROUP TEXT', "getGroup's own groupProfile reflects the reset")
  assert(
    (res.json.latestDnaVersion.content as { groupProfile: string }).groupProfile === 'ORIGINAL GROUP TEXT',
    "getGroup's latestDnaVersion also reflects the reset, not the stale AI-generated text the Admin's detail view was still showing",
  )

  // Gone from both the Chair's and the NA's own lists — exactly like a
  // group that was never launched.
  res = await call('/api/getChairGroups', { method: 'GET', email: chairEmail })
  assert(!(res.json.groups as Array<{ id: string }>).some((g) => g.id === groupWithSnapshot.id), "the reset group no longer appears in the Chair's list")
  res = await call('/api/getNaGroups', { method: 'GET', email: naEmail })
  assert(!(res.json.groups as Array<{ id: string }>).some((g) => g.id === groupWithSnapshot.id), "the reset group no longer appears in the NA's list")
  console.log('  5. Reset fully unlaunches: status back to Imported, approvals cleared, fresh DnaVersion, old NA comment + chat history cleared, gone from Chair/NA lists ok')
